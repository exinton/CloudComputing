 ...
 * @author Avinash
 */
public class CrushRunNoFailDisk {

    private final ArrayList<CrushLevel> crushLevels;
    private static CrushRunNoFailDisk instance = null;

    public static CrushRunNoFailDisk getInstance() {
        if (instance == null) {
            instance = new CrushRunNoFailDisk();
        }
        return instance;
    }

    protected CrushRunNoFailDisk() {
    ...
    public ArrayList<CephNode> runCrush(CephMap cephmap, String objectName, String opType, boolean... addnFlags) {
        ...
        if (!isBalanced) {
            try {
                ...
                Logger.getLogger(CrushRunNoFailDisk.class.getName()).log(Level.SEVERE, null, ex);
                return new ArrayList<CephNode>();
            }
        }
...
//                        System.out.println("select::bucketOutput:"+bucketOutput.getId());
                        ...
                        if (bucketOutput.getLevelNo() != outputLevelNo) {
                            //b,<- bucket(o)
                            bucket = bucketOutput.getChildren();
                        } else if (output.contains(bucketOutput)|| overload(bucketOutput, opType, fileName)) {
