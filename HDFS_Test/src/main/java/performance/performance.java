...
public class performance {
		...
		try {
			...
			while ((tempString = reader.readLine()) != null) {
				if (tempString.startsWith("Success")) {
					...
					String[] one_request_record = { request_type, target_file,
							offset_start, offset_end, start_time, end_time };
	...
	public void print_all_request_record() {
		for (int i = 0; i < request_log.size(); i++) {
			for (int j = 0; j < request_log.get(i).length; j++) {
				...
				System.out.println(i + " Request type : "
						+ one_request_record[0]);
				System.out
						.println("    target_file : " + one_request_record[1]);
				System.out.println("    offset_start => offset_end : "
						+ one_request_record[2] + " => "
						+ one_request_record[3]);
				System.out.println("    start_time => end_time : "
						+ one_request_record[4] + " => "
						+ one_request_record[5]);
			}
		}
	}
	...
	public double get_reading_speed() {
		...
		for (int i = 0; i < request_log.size(); i++) {
			...
			long size = Long.valueOf(request_log.get(i)[3])
					- Long.valueOf(request_log.get(i)[2]);
			if (type.contains("SEQ_READ")) {
				size = Long.valueOf(request_log.get(i)[3]) - Long.valueOf(0);
			}
			long time = tt.get_time_gap(request_log.get(i)[4],
					request_log.get(i)[5]);
	...
	public double get_writing_speed() {
		...
		for (int i = 0; i < request_log.size(); i++) {
			...
			long time = tt.get_time_gap(request_log.get(i)[4],
					request_log.get(i)[5]);
			if (type.contains("CREATE") || type.contains("APPEND")
					|| type.contains("SEQ_WRITE")
					|| type.contains("RANDOM_WRITE")) {
				total_size += size;
				total_time += time;
			}
		}
	...
	public double get_throughput() {
		...
		// System.out.println(String.valueOf(total_size)+" "+start_time_string+" "+end_time_string+" "+total_size);
		return throughput;
	}

	public void get_request_distribution(){
		HashMap<String,Integer> h = new HashMap<String,Integer>();
		for (int i = 0; i < request_log.size(); i++) {
			String file = request_log.get(i)[1];
			if(h.containsKey(file)){
				   h.put(file, h.get(file)+1);
			   }else{
				   h.put(file, 1);
			   }
		}
